...
Technical standards and architectural constraints for Claude when working in this repository.
...
- **Vendor Name:** `jweiland/service-bw2`.
- **Namespace:** `JWeiland\ServiceBw2`
- **Test Namespace:** `JWeiland\ServiceBw2\Tests`
...
- **Automation:**
    ...
    - Use the provided `./Build/Scripts/runTests.sh` for executing tests, linting, and code style fixes.
    - Locally always prefix with `CI=true` to avoid TTY issues: `CI=true ./Build/Scripts/runTests.sh -s functional`.
    - Run `cgl` before every commit to auto-fix code style issues.
    - All available test suites and example invocations are documented in `.claude/testing.md`.
...
When asked to create a git commit, follow the guidelines in `.claude/commit.md`.