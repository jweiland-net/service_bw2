 ...
 *     ...
 * </dl>
 * Displays all opening hours using 24-hour system
 */
final class OpeningHoursViewHelper extends AbstractViewHelper
{
    ...
    public function render(): string
    {
        ...
        foreach ($this->arguments['oeffnungszeitenStrukturiert'] as $structuredOpeningHours) {
            if (
                isset($structuredOpeningHours['typ'])
                && in_array($structuredOpeningHours['typ'], $allowedTypes, true)
            ) {
                $html[] = $this->getStructuredOpeningHoursHTML($structuredOpeningHours);
            }
        }
